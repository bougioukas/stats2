...
First, we will use the `plot()` function. In R, this is a basic function used to create graphs and visualize data. Since it is part of base R, we do not need to install any additional packages to use it. 

```{r}
# Basic plot
plot(dat10$age, dat10$shock_index,           # <1>
...
1. `dat10$` tells R which dataset the variable belongs to and selects that specific column.
...
```{r}
ggplot(dat10, aes(x = age, y = shock_index)) +    # <1>
  geom_point(size = 2.5) +                        # <2>
...
1. `aes()` stands for aesthetic mappings. It defines how variables from the dataset are mapped to visual properties of the plot, such as x-axis position and y-axis position.

2. `geom_point()` is a geometry function that draws points on the plot.
